...
const SelectedRefinements = ({
  ...
  const priceFilterValues = filters?.find(
    (filter) => filter.attributeId === 'price'
  );
  ...
  for (const key in selectedFilterValues) {
    console.log('selected-refinements-split-start');
    const filters = selectedFilterValues[key].split('|');
    console.log('selected-refinements-split-end');
